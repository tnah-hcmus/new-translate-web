...
import {
  addTab,
  deleteTabWCloud,
  updateCommentsWCloud,
  updateTabWCloud,
} from "../../../actions/tabs/tabs_action";
import { setCreditWCloud } from "../../../actions/credit/credit_action";
import {
  replaceTabID,
  addReplies,
} from "../../../actions/replies/replies_action";
import CommentPreview from "../comment/CommentPreview";
const TitlePreview = lazy(() =>
  import(/* webpackChunkName: "TitlePreview" */ "./TitlePreview")
);
const SectionHeader = lazy(() =>
  import(/* webpackChunkName: "SectionHeader" */ "./SectionHeader")
);
const PreviewModal = lazy(() =>
  import(/* webpackChunkName: "PreviewModal" */ "../modal/PreviewModal")
);
...
class Section extends React.PureComponent {
  ...
  reCrawl = () => {
    crawler(
      this.props.tab.link.replace(/\?[^?]+$/, ""),
      this.updateProgressBar,
      this.state.fullCrawl
    )
      .then((result) => {
    ...
    if (this.props.tab.link) {
      ...
      this.checkPostCommentExist(this.props.tab.id, idb)
        .then((result) => {
          if (
            result &&
            this.props.replies[this.props.tab.id] &&
            this.props.replies[this.props.tab.id].length > 0
          ) {
            let dataIsValid = false;
            const store = idb[this.props.tab.id];
            store
              .get(this.props.replies[this.props.tab.id][0].id)
              .then((data) => {
                if(data) dataIsValid = true;
              })
              .catch((err) => {
                console.log(err);
              })
              .finally(() => {
                if (dataIsValid) {
                  ...
                  idb
                    .deleteObjectStore(this.props.tab.id)
                    .then(() => {
                      this.reCrawl();
                    })
  ...
  fullCommentSearch = async (searchKey, mode, replies) => {
    ...
    const result = await this.searchCommentIndexedDB(
      idb,
      master,
      searchKey,
      mode
  ...
  handleSubmitLink = (event) => {
    ...
    let regex = new RegExp(
      "https?://(?:www.|(?!www))reddit.[^s]{2,}|www.reddit.[^s]{2,}|https?://(?:www.|(?!www))reddit.[^s]{2,}|www.reddit.[^s]{2,}|(?!www)redd.it/[^s]{2,}"
    );
  ...
  handleSubmitCredit = (event) => {
    ...
    saveDraft(this.state.info.id, this.props.uuid, {
      timemark: Date.now(),
      credit: credit && credit !== "" ? credit : "Một member chăm chỉ nào đó",
    });
  };
  ...
  addTransComment = (
    ...
    if (parent) {
      for (let i in parent) {
        if (!trans[parent[i]])
          this.addTransComment(
            parent[i],
            parseInt(i) + 1,
            null,
            parent.slice(0, parseInt(i)),
            null,
            null,
            [...parent.slice(parseInt(i) + 1, parent.length), id]
          );
        else {
    ...
    this.setState((preState) => {
      ...
      if (
        transComment[id].prefixed === null ||
        transComment[id].prefixed === ""
      ) {
    ...
    comments.children.map((id) => {
      if (array[id]?.level === level)
        content =
          content +
          endLine +
          this.getTransReplies("", array[id], array, level + 1);
    });
    if (!(comments.body.trim() === ""))
      content =
        comments.prefixed +
  ...
  previewContent = () => {
    ...
    if (info.id) {
      ...
      let content =
        ...
        endLine +
        info.author +
        ` (${info.upvotes}${info.awards && " - "}${info.awards}) ` +
        endLine;
      ...
      //Thêm note và credit
      if (this.state.note !== "")
        content = content + this.state.note + endLine + commentSeparator;
      content =
        content +
        `Dịch bởi ${this.state.credit || "một member chăm chỉ dịch bài"}` +
        endLine +
  ...
  afterRestore = () => {
    ...
    for (let id in trans) {
      if (trans[id].author && trans[id]?.level > 0)
        showInput =
          showInput + trans[id].prefixed + trans[id].author + authorSeparator; //thêm comment để hiện sau khi restore
    }
  ...
  savePost = (first, input = {}) => {
    ...
    const saveData = input.data || {
      timemark: Date.now(),
      credit:
        this.state.credit && this.state.credit !== ""
          ? this.state.credit
          : "Một member chăm chỉ nào đó",
    };
    updateState(1);
    this.changed = false;
    if (this.state.link) {
      ...
      if (
        this.props.tab.category === "blank" ||
        this.props.tab.category !== data.category
      ) {
  ...
  render() {
    return (
      <section
        className={
          "section js-section " +
          (this.props.activeSection === this.props.tab.id ? "is-shown" : "")
        }
      >
        <Suspense
          fallback={<div></div>}
          key={this.props.tab.id + "-header-suspense"}
        >
          <SectionHeader
        ...
        <InputContext.Provider
          value={{
            addTransComment: this.addTransComment,
            editTransComment: this.editTransComment,
            changed: this.handleChanged,
          }}
        >
          <div
            className={
              this.state.info.id ? "title-panel shown" : "title-panel hide"
            }
          >
            <Suspense
              fallback={<div></div>}
              key={this.state.info.id + "-title-suspense"}
            >
              <TitlePreview
          ...
          <p
            className={"restore" + (this.state.isReady ? " hide" : "")}
            style={{ textAlign: "center" }}
          >
            Restoring your trans comments, hold your apple...
          </p>
          <div
            id={this.props.tab.id + "-panel"}
            className={"panel" + (this.state.isReady ? " shown" : "")}
            style={{
              paddingBottom: this.state.comments.length ? "3em" : "6em",
            }}
          >
            {this.state.comments.length !== 0 || this.state.inSearch ? (
              <div style={{ display: "flex", flexWrap: "wrap" }}>
                ...
                <button
                  ...
                  onClick={() => this.searchComment(this.state.search)}
                >
                  Search
                </button>
              </div>
              ...
              <div className="progress">
                {!this.state.isCrawling ? (
                  <p className="widget__message">
                    Không biết nên dịch gì? Gợi ý một số post nhé!
                  </p>
                ) : this.state.progressBarInfo === null ? (
                  <CircularProgress style={{ color: "var(--color-accent)" }} />
                ) : (
                  <CircularProgressWithLabel
                    value={this.state.progressBarInfo}
                  />
                )}
              </div>
            )}
        ...
        <Suspense
          fallback={<div></div>}
          key={this.state.info.id + "-modal-suspense"}
        >
          <PreviewModal
...
const mapDispatchToProps = {
  addTab,
  deleteTab: deleteTabWCloud,
  updateTab: updateTabWCloud,
  addCategory: addCategoryWCloud,
  updateComments: updateCommentsWCloud,
  replaceTabID,
  addReplies,
  setCreditWCloud,
};
